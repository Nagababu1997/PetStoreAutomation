...
public class StudentTest {
    ...
    void setUpData() {
        ...
        userPayload.setId(faker.idNumber().valid());
        userPayload.setName(faker.name().fullName());
        userPayload.setLocation(faker.country().name());
...
// Pick 2 random ones
        ...
        userPayload.setCourses(selectedCourses);
...
//        logs
    ...
    public void testGetStudent() {
        ...
        Response response = StudentEndPoints.getStudent(this.userPayload.getId());
        response.then().log().all();
...
//        updating the data by payload
        ...
        userPayload.setLocation(faker.country().name());
        userPayload.setPhone(faker.phoneNumber().phoneNumber());
        Response response = StudentEndPoints.updateStudent(this.userPayload.getId(), userPayload);
        response.then().log().all();

        Assert.assertEquals(response.getStatusCode(), 200);

//        checking student details after updating
        Response responseUpdatedStudent = StudentEndPoints.getStudent(this.userPayload.getId());
        response.then().log().all();
    ...
    public void testDeleteStudent() {
        ...
        Response response = StudentEndPoints.deleteStudent(this.userPayload.getId());
        Assert.assertEquals(response.getStatusCode(), 200);
